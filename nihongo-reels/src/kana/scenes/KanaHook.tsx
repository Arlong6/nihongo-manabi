...
export const KanaHook: React.FC<{ p: KanaPair }> = ({ p }) => {
  ...
  const leftEnter = spring({ frame, fps, config: { damping: 9, stiffness: 240 } });
  const rightEnter = spring({ frame: frame - 6, fps, config: { damping: 9, stiffness: 240 } });
  ...
  return (
    <AbsoluteFill style={{ background: kanaTheme.bg, padding: 80, justifyContent: "center", alignItems: "center", gap: 40 }}>
      ...
      <div style={{ color: kanaTheme.text, fontFamily: kanaTheme.fontZh, fontSize: 64, fontWeight: 800, letterSpacing: 4 }}>
        分得出這兩個嗎？
      </div>
